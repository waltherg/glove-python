...
class Glove(object):
    ...
    @classmethod
    def load_stanford(cls, filename, no_components=300,
                      no_vectors=2196017):
        """
        ...
        dct = {}
        vectors = np.zeros(shape=(no_vectors, no_components), dtype=np.float16)
        
        # Read in the data.
        skipped = 0
        with open(filename, 'r') as savefile:
            for i, bline in enumerate(savefile):
                try:
                ...
                dct[word] = i
                vectors[i, :] = list(entries)

        print('Glove.load_stanford skipped {} lines'.format(skipped))
